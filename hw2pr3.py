...
def find_RMSE(W, X, y):
	...
	#"*** YOUR CODE HERE ***"
	yPrediction = predict(W, X)
	difference = y - yPrediction
	slope = X.shape[0]
	MSE = np.linalg.norm(difference, 2) ** 2 / slope
	RMSE = np.sqrt(MSE)
	#"*** END YOUR CODE HERE ***"
	return RMSE
...
def linreg_no_bias(X, y, reg=0.0):
	...
	W_opt = np.linalg.solve((X.T @ (np.eye(m) - np.ones(m) / m)) @ X + reg * np.eye(Aggregate.shape[0]), \
		(X.T @ (np.eye(m) - np.ones(m) / m)) @ y)
